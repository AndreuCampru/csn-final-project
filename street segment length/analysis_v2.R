...
plot_model <- function(model_spec, params, df, dens, x_scale = "log10", y_scale = "log10") {
  ...
  ggplot() +
    ...
    scale_x_log10() +
    labs(
      title = paste("Barcelona's Street Length Distribution -", model_spec$id),
      x = "Street Length (m)",
      y = "Density"
    ) +
    theme_minimal() +
...
# Model 6 Specification with an expanded starting grid
model6_spec <- list(
  id = "Model 6",
...
for (model_spec in model_specs) {
  cat("Estimating", model_spec$id, "\n")
  res <- run_model_estimation(model_spec, df)
  results[[model_spec$id]] <- res
  cat("Best parameters for", model_spec$id, ":", res$best_params, "\n")
  cat("Minimum negative log-likelihood:", res$min_neg_loglik, "\n\n")
  
  # Plot the best model
  print(plot_model(model_spec, res$best_params, df, dens))
}

################ Generate Latex tables ################ 

# Load necessary package for LaTeX table conversion
library(xtable)

# Assuming 'model_specs' list and 'results' list are available from previous estimation steps.
# Also assuming get_AIC function is defined.

N <- length(df$length)  # sample size

# Data frame to store summary for each model
...
# Iterate over each model specification to collect parameters and AIC
for(model_spec in model_specs) {
  res <- results[[model_spec$id]]
  if(!is.null(res)) {
    # Extract best parameters and format as a string
    params <- res$best_params
    param_str <- paste(names(params), sprintf("%.4g", params), collapse = ", ")
    
    # Determine number of parameters K and compute AIC
...
# Determine the minimum AIC across models
AIC_best <- min(model_summary$AIC, na.rm = TRUE)

# Calculate AIC difference for each model
model_summary$AIC_diff <- model_summary$AIC - AIC_best

# Create separate tables: one for parameters and one for AIC differences
parameters_table <- model_summary[, c("Model", "Parameters")]
aic_diff_table <- model_summary[, c("Model", "AIC_diff")]

# Convert tables to LaTeX using xtable
latex_parameters <- xtable(parameters_table, caption = "Estimated Parameters for Each Model")
latex_aic_diff   <- xtable(aic_diff_table, caption = "AIC Differences for Each Model")

# Export LaTeX code to files instead of printing to console
