...
if (!dir.exists("../data/processed/latex")) {
  dir.create("../data/processed/latex", recursive = TRUE)
}
...
# Define main folder for all models
main_vis_path <- "../visualisations/models"

# Create main folder if it doesn't exist
if (!dir.exists(main_vis_path)) {
  dir.create(main_vis_path)
}
# Loop over each city
for(city in cities) {
  ...
  df <- read.csv(paste0("../data/processed/street_segment_length/", city, "_street_lengths.csv"))
  names(df)[1] <- "length"
  ...
  # Create subfolder for the current city
  city_vis_path <- file.path(main_vis_path, city)
  if (!dir.exists(city_vis_path)) {
    dir.create(city_vis_path)
  }
  ...
  for(model_spec in model_specs) {
    ...
    if(is.null(res$best_params)) {
      ...
      filename1 <- paste0(city_vis_path, "/", model_spec$id, "_no_logy.png")
      ...
      filename2 <- paste0(city_vis_path, "/", model_spec$id, "_logy.png")
...
# Save LaTeX files to data/processed/latex
print(latex_parameters, include.rownames = FALSE,
      file = "../data/processed/latex/parameters_table.tex")
print(latex_aic_diff, include.rownames = FALSE,
      file = "../data/processed/latex/aic_diff_table.tex")